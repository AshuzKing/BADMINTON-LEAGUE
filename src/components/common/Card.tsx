...
export const Card = ({ children, className, onClick }: { children: React.ReactNode, className?: string, onClick?: () => void }) => {
  return (
    <div 
      onClick={onClick}
      className={twMerge("bg-darkBlue/50 border border-white/10 p-6 relative group overflow-hidden transition-all hover:border-electricBlue/50", onClick && "cursor-pointer hover:transform hover:-translate-y-1", className)}
    >
